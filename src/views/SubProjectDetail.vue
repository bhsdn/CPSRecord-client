<template>
      ...
      <el-timeline v-else>
        ...
        >
          <div class="rounded-lg border border-slate-200 bg-white p-3">
            ...
            <div class="mt-2">
              <template v-if="content.contentType.fieldType === 'image'">
                <img
                  v-lazyload="content.contentValue"
                  alt="内容图片"
                  class="h-24 w-24 rounded-lg border border-slate-200 object-cover shadow-sm"
                />
              </template>
              <template v-else-if="content.contentType.fieldType === 'url'">
                <a
                  :href="content.contentValue"
                  target="_blank"
                  rel="noopener"
                  class="break-words text-sm text-primary-600 underline"
                >
                  {{ content.contentValue }}
                </a>
              </template>
              <template v-else>
                <p class="whitespace-pre-wrap break-words text-sm text-slate-700">
                  {{ content.contentValue }}
                </p>
              </template>
            </div>
  ...
  <ConfirmDialog
    ...
    title="批量删除过期口令"
    :description="`确认删除选中的 ${bulkDeleteCommands.length} 个过期口令吗？`"
    confirm-text="确认删除"
    :loading="submitting"
    @confirm="handleBulkDeleteCommands"
...
const textCommandListRef = ref<InstanceType<typeof TextCommandList> | null>(null);
...
const bulkDeleteCommands = ref<TextCommand[]>([]);
...
const bulkDeleteDialogVisible = computed({
  get: () => dialogStates.bulkDelete,
  set: (value: boolean) => (dialogStates.bulkDelete = value),
});
...
const prepareBulkDeleteCommands = (commands: TextCommand[]) => {
  if (!commands.length) return;
  bulkDeleteCommands.value = commands;
...
const handleBulkDeleteCommands = async () => {
  if (!subProject.value || !bulkDeleteCommands.value.length) return;
  submitting.value = true;
  try {
    await contentsStore.removeTextCommands(
      subProject.value.id,
      bulkDeleteCommands.value.map((command) => command.id)
    );
    ElMessage.success(`已删除 ${bulkDeleteCommands.value.length} 个口令`);
    textCommandListRef.value?.clearSelection();
  } catch (error) {
    ElMessage.error("批量删除失败");
  } finally {
    submitting.value = false;
    bulkDeleteDialogVisible.value = false;
    bulkDeleteCommands.value = [];
  }
};
